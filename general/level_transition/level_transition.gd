...
func _ready() -> void:
	if Engine.is_editor_hint():
		return
	SceneManager.load_scene_ready.connect(_on_new_scene_ready)
...
func _on_new_scene_ready(_target_name: String, offset : Vector2) -> void:
	# Player Position
	if _target_name == name:
...
func _on_load_scene_finished() -> void:
	# Resolve transition timing between scenes cause it lose 2 frames and area continue to monitor
	# So disable monitoring, skip 2 frames and enable monitoring again
